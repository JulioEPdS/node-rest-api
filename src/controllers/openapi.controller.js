...
export const querySignup = async(req,res) => {
    ...
    const {
        ...
        phone, gender,
        discapacidad, eres,
        empresa, producto_servicio,
        birthdate,  rfc,
        curp,  compartir
    } = req.body
    const id = uuidv4()
    if (email && password && nombre && paterno && materno && direccion && estado && municipio &&
        phone && gender && discapacidad && eres && empresa && producto_servicio && birthdate && 
        rfc && curp && compartir){
        try{
            const pool = await getConnection()
            await pool
...
export const queryRegistro = async(req,res)=>{ 
    ...
    const {
        email,nombre,paterno,
        materno,direccion,estado,
        municipio,phone,gender,
        discapacidad,eres,empresa,
        producto_servicio,birthdate,
        rfc,curp,compartir
    } = req.body
    const id = uuidv4()
    if(email && nombre && paterno && materno && direccion && estado && municipio &&
        phone && gender && discapacidad && eres && empresa && producto_servicio &&
        birthdate && rfc && curp && compartir && id){
        try{
            const pool = await getConnection()
            await pool
            .request()
